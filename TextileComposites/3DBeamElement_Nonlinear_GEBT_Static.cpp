...
int VAMBeamElement::get_ndim()
{
    return this->NDM;
}

double VAMBeamElement::get_diameter()
{
    return this->D;
}

Eigen::VectorXd VAMBeamElement::get_axis()
{
    return this->axis;
}
    ...
    if (nodenum == 0)
    {
        ...
        * Transform_Mat(Theta)* kappa1;
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            R(i + 6) = fF_minus(i) - a1(i);
            R(i + 9) = fM_minus(i) - b1(i);
        }
...
Eigen::MatrixXd VAMBeamElement::Element_Jacobian(Eigen::VectorXd U1, Eigen::VectorXd F1, double h, Eigen::MatrixXd S1, double Theta, int nodenum,
    std::fstream& file1)
{
    Eigen::VectorXd F_1(3), M_1(3), u1(3), theta1(3), Strain1(6), gamma1(3), kappa1(3), e_1(3);
    Eigen::MatrixXd dgammadF(3, 3), dgammadM(3, 3), dkappadF(3, 3), dkappadM(3, 3);
...
//Eigen::VectorXd VAMBeamElement::Update_Strains(VAMBeamElement* VAMBE, int nbeams, Eigen::VectorXd* U, std::fstream& file1)
//{
//    int size = 0;
//    for (int i = 0; i < nbeams; i++)
//        size += VAMBE[i].get_nnode() * 6;
...
//        int temp = 0;
//        for (int i = 0; i < m; i++)
//            temp += VAMBE[i].get_nnode() * 6;
//        int nnode = VAMBE[m].get_nnode();
//        for (int i = 0; i < nnode - 1; i++)
//        {
//            Eigen::VectorXd Force(6);
//            for (int j = 0; j < 6; j++)
//                Force(j) = (*U)(12 * i + j + temp);
...
//            do
//            {
//                Seq = VAMBE[m].Equivalent_StiffnessMatrix_FirstOrder(Strain, VAMBE[m].get_inittwist(), VAMBE[m].get_width(), file1);
//                Sinv = Seq.inverse();
...
//Eigen::VectorXd VAMBeamElement::Update_Strains(VAMBeamElement VAMBE, Eigen::VectorXd* U, std::fstream& file1)
//{
//    int nnode = VAMBE.get_nnode();
//    Eigen::MatrixXd Seq = Eigen::MatrixXd::Zero(6, 6);
//    Eigen::MatrixXd Sinv = Eigen::MatrixXd::Zero(6, 6);
...
//        do
//        {
//            Seq = Equivalent_StiffnessMatrix_FirstOrder(Strain, VAMBE.get_inittwist(), VAMBE.get_width(), file1);
//            Sinv = Seq.inverse();
//            Strain_new = Sinv * Force;
//            Eigen::VectorXd error(6);
//            for (int j = 0; j < 6; j++)
