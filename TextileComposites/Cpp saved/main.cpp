...
int main()
{
    ...
    else if (choice == 6)
    {
        ...
        {
            EBBE3D[i] = NLEBBE3D(nbeams, 4.5, 1, 4, "LINELAS", i + 1, 50, 3, 3);
                ...
                do
                {
                    ...
                    for (int l = 0; l < nbeams; l++)
                    {
                        ...
                        for (int i = 0; i < VAMBE[l].get_nelem(); i++)
                        {
                            ...
                            double xa = VAMBE[l].get_coordinates(a, 0);
                            double xb = VAMBE[l].get_coordinates(b, 0);
                            double h = xb - xa;
                            if (i == 0)
                            {
                                Eigen::VectorXd Element_R = Eigen::VectorXd::Zero(12);
                                Eigen::MatrixXd Element_J = Eigen::MatrixXd::Zero(12, 12);
                                ...
                                for (int j = 0; j < 6; j++)
                                {
                                    ...
                                    F1(j) = U(a * 12 + j + temp);
                                    //u, theta
                                    U1(j) = U(a * 12 + j + 6 + temp);
                                }

                                Seq_1 = VAMBE[l].Equivalent_StiffnessMatrix_FirstOrder(file1);

                                Element_R = VAMBE[l].Element_Residual(F1, U1, h, Seq_1, Theta);
                                Element_J = VAMBE[l].Element_Jacobian(F1, U1, h, Seq_1, Theta, file1);
                        ...
                        for (int i = 0; i < ConPair.size(); i++)
                        {
                            ...
                            if (g < 0)
                            {
                                for (int l = 0; l < 6; l++)
                                {
                                    ...
                                    for (int m = 0; m < 6; m++)
                                    {
                                        J.coeffRef(VAMBE[mbeam].get_ndof() * mnode1 + l + temp1, VAMBE[mbeam].get_ndof() * (mnode1 - 1) + m + temp1 + 6) += CT[l][m];
                                        J.coeffRef(VAMBE[sbeam].get_ndof() * mnode1 + l + temp1, VAMBE[mbeam].get_ndof() * (snode1 - 1) + m + temp2 + 6) += CT[l][m + 6];
                                        J.coeffRef(VAMBE[mbeam].get_ndof() * snode1 + l + temp2, VAMBE[sbeam].get_ndof() * (mnode1 - 1) + m + temp1 + 6) += CT[l + 6][m];
                                        J.coeffRef(VAMBE[mbeam].get_ndof() * snode1 + l + temp2, VAMBE[mbeam].get_ndof() * (snode1 - 1) + m + temp2 + 6) += CT[l + 6][m + 6];
                                    }
                                }
                            }
                        }
